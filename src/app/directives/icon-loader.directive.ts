...
export class IconLoaderDirective implements OnInit{
  ...
  ngOnInit() {
    this.el.nativeElement.title = (this.appIconLoader ?? '') + (this.iconSize ?? '');
    this.el.nativeElement.src = 'https://openweathermap.org/img/wn/'+(this.appIconLoader ?? '') + '@'+(this.iconSize ?? '')+'x.png'
  }
}
